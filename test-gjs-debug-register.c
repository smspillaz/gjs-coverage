...
{
  GjsDebugInterruptRegisterFixture *fixture = fixture_data;
  GjsDebugInterruptRegisterConnectionFunction connector = user_data;
  GjsDebugConnection *connection =
      add_dummy_connection_from_function (fixture, connector);
  JSContext *js_context = gjs_context_get_native_context (fixture->context);
                                                                         ...
                                                                         gpointer             user_data)
{
  ...
  *function_names_hit = g_list_append (*function_names_hit, g_strdup (info->functionName));
}
  ...
  for (; i < n_elements; ++i)
    {
      ...
      while (iter)
        {
          if (g_strcmp0 (iter->data, elements[i]) == 0)
            found = TRUE;
                                                  ...
                                                  gconstpointer user_data)
{
  ...
  g_assert (last_loaded_script != NULL &&
            g_strcmp0 (last_loaded_script, fixture->temporary_js_script_filename) == 0);
...
static void
for_each_in_table_driven_test_data (gpointer      test_data,
                                    gsize         element_size,
                                    gsize         n_elements,
                                    TestDataFunc  func,
                                    gconstpointer user_data)
{
  gpointer test_data_iterator = test_data;
  gsize i = 0;
  for (; i < n_elements; ++i, test_data_iterator += element_size)
    (*func) (test_data_iterator, user_data);
}
...
add_test_for_fixture_size_and_funcs (gpointer      data,
                                     gconstpointer user_data)
{
  const FixturedTableDrivenTestData *fixtured_table_driven_test = data;
...
add_gjs_debug_interrupt_register_context_state_data_test (gpointer      data,
                                                          gconstpointer user_data)
{
  const GjsDebugInterruptRegisterTableDrivenTest  *test = user_data;
  GjsDebugInterruptRegisterContextStateData       *table_data = data;
  ...
  add_test_for_fixture_size_and_funcs (&fixtured_data, table_data->connector);
          ...
          char **argv)
{
  ...
  for (; i < interrupt_register_tests_info_size; ++i)
    for_each_in_table_driven_test_data (&context_state_data,
                                        ...
                                        &interrupt_register_tests_info[i]);
